...
| · | &nbsp;&nbsp;`Admin.config` | 103 |  |
| · | &nbsp;&nbsp;`Admin.values` | 106 |  |
| · | &nbsp;&nbsp;`Admin.add_export` | 111 |  |
| · | &nbsp;&nbsp;`Admin.remove_export` | 142 |  |
| · | &nbsp;&nbsp;`Admin.columns` | 159 |  |
| · | &nbsp;&nbsp;`Admin.test_export` | 182 |  |
| · | &nbsp;&nbsp;`Admin.save` | 198 |  |
...
| f | `page` | 410 |  |
| **C** | `_Handler` | 779 |  |
| · | &nbsp;&nbsp;`_Handler.log_message` | 783 |  |
| · | &nbsp;&nbsp;`_Handler.do_GET` | 846 |  |
| · | &nbsp;&nbsp;`_Handler.do_POST` | 859 |  |
| · | &nbsp;&nbsp;`_Handler.do_HEAD` | 978 |  |
...
| f | `importer` | 548 | The bring-it-over-from-WeeWX form. Three ways in, and the order matters. A file is the one that works from any |
...
| · | &nbsp;&nbsp;`FirstLast.set_stats` | 73 |  |
| · | &nbsp;&nbsp;`FirstLast.stats_tuple` | 76 |  |
| · | &nbsp;&nbsp;`FirstLast.merge_hilo` | 79 |  |
| · | &nbsp;&nbsp;`FirstLast.merge_sum` | 89 |  |
| · | &nbsp;&nbsp;`FirstLast.add_hilo` | 92 |  |
| · | &nbsp;&nbsp;`FirstLast.add_sum` | 102 |  |
| · | &nbsp;&nbsp;`FirstLast.avg` | 106 |  |
| **C** | `Scalar` | 110 | Statistics for a scalar observation: min, max, and a weighted mean. `wsum` and `sumtime` are what make an aver |
| · | &nbsp;&nbsp;`Scalar.__init__` | 120 |  |
...
| · | &nbsp;&nbsp;`Scalar.add_sum` | 163 |  |
| · | &nbsp;&nbsp;`Scalar.avg` | 173 |  |
| **C** | `Vector` | 177 | Statistics for wind: scalar stats plus the vector sums. `xsum`/`ysum` accumulate the east and north components |
| · | &nbsp;&nbsp;`Vector.__init__` | 192 |  |
...
| · | &nbsp;&nbsp;`Accumulator.add_record` | 332 |  |
| · | &nbsp;&nbsp;`Accumulator.merge_hilo` | 356 |  |
| · | &nbsp;&nbsp;`Accumulator.record` | 368 |  |
| · | &nbsp;&nbsp;`Accumulator.augment` | 372 |  |
| · | &nbsp;&nbsp;`Accumulator.set_stats` | 384 |  |
...
| · | &nbsp;&nbsp;`Archiver.build` | 83 |  |
| · | &nbsp;&nbsp;`Archiver.store` | 146 |  |
| · | &nbsp;&nbsp;`Archiver.process_due` | 174 |  |
| · | &nbsp;&nbsp;`Archiver.catch_up` | 199 |  |
| · | &nbsp;&nbsp;`Archiver.rebuild` | 227 |  |
| · | &nbsp;&nbsp;`Archiver.run` | 266 |  |
...
| f | `read_config` | 177 | The whole configuration file, or an empty one. A file that is not there yet is not an error: a first start has |
| f | `configure_drivers` | 188 | Hand each driver its options, and somewhere to keep its own state. Until this runs the drivers are unconfigure |
...
| f | `cmd_listen` | 290 |  |
| f | `cmd_archive` | 334 |  |
| f | `cmd_serve` | 393 | Listener and archiver in one process, for a small machine. They still speak only through the database. Splitti |
| f | `cmd_catchup` | 557 |  |
| f | `cmd_rebuild` | 568 |  |
| f | `cmd_columns` | 579 | Readings that the archive has no column for. A reading only survives the archive interval if the table has a c |
| f | `cmd_driver_list` | 654 |  |
| f | `cmd_driver_install` | 676 |  |
| f | `cmd_driver_remove` | 710 |  |
| f | `local_addresses` | 719 | Addresses this machine can be reached on, as (address, what it is). Only what can be worked out without asking |
| f | `cmd_url` | 751 | Print the addresses of the live view and the upload endpoint. The token is a path segment, so an address witho |
...
| f | `all_schemas` | 838 | Every configurable thing there is: the core, then each driver. Drivers are asked, not enumerated. One that gai |
| f | `replace_group` | 901 |  |
...
| f | `cmd_admin` | 1000 | Serve the settings page. Its own port and its own token, deliberately. An upload endpoint can at worst put a w |
...
| f | `load_plots` | 1272 |  |
| f | `build_feeds` | 1276 | The feeds this configuration asks for, in the order they run. The JSON first, because the diagnostic page draw |
...
| · | &nbsp;&nbsp;`ArchiveStore.reload_schema` | 66 |  |
| · | &nbsp;&nbsp;`ArchiveStore.last_timestamp` | 116 |  |
| · | &nbsp;&nbsp;`ArchiveStore.count` | 119 |  |
| · | &nbsp;&nbsp;`ArchiveStore.exists` | 122 |  |
| · | &nbsp;&nbsp;`ArchiveStore.record` | 127 |  |
| · | &nbsp;&nbsp;`ArchiveStore.add_record` | 139 |  |
| · | &nbsp;&nbsp;`ArchiveStore.homeless` | 198 |  |
| · | &nbsp;&nbsp;`ArchiveStore.add_column` | 202 |  |
| · | &nbsp;&nbsp;`ArchiveStore.add_records` | 225 |  |
| · | &nbsp;&nbsp;`ArchiveStore.rebuild_day` | 342 |  |
| · | &nbsp;&nbsp;`ArchiveStore.days` | 377 |  |
| · | &nbsp;&nbsp;`ArchiveStore.set_meta` | 390 |  |
...
| f | `weight_of` | 27 | The weight one archive record carries in a daily summary. WeeWX uses this for daily-summary version 2.0 and up |
| f | `day_accumulator` | 41 | An accumulator spanning one archive day, starting at `sod_ts`. |
| f | `build` | 47 | Fold archive records into one accumulator per day. Records must arrive in ascending time order -- the same ord |
| f | `read_day` | 82 | Read one day's stored statistics for one observation type. |
| f | `read_records` | 94 | Read archive records in time order, dropping the columns that are NULL. Dropping nulls matters: the accumulato |
...
| | Name | Zeile | |
|---|---|---|---|
| f | `interval_stop` | 67 | The end of the archive interval a timestamp belongs to. Intervals are half-open at the start, so a packet stam |
...
| · | &nbsp;&nbsp;`LiveStore.close` | 168 |  |
| · | &nbsp;&nbsp;`LiveStore.add` | 188 |  |
| · | &nbsp;&nbsp;`LiveStore.add_all` | 210 |  |
| · | &nbsp;&nbsp;`LiveStore.mark_pending` | 216 |  |
| · | &nbsp;&nbsp;`LiveStore.clear_pending` | 225 |  |
| · | &nbsp;&nbsp;`LiveStore.due` | 230 |  |
| · | &nbsp;&nbsp;`LiveStore.packets` | 246 |  |
| · | &nbsp;&nbsp;`LiveStore.forget_raw` | 269 |  |
| · | &nbsp;&nbsp;`LiveStore.raw_of` | 281 |  |
| · | &nbsp;&nbsp;`LiveStore.span` | 292 |  |
| · | &nbsp;&nbsp;`LiveStore.count` | 295 |  |
...
| f | `windchill_c` | 125 | Wind chill, Celsius, from the Environment Canada formula. Only defined below 10 degrees and above 4.8 km/h. Ou |
| f | `heatindex_f` | 139 | Heat index, Fahrenheit, Rothfusz with the two adjustments. Below 40 F there is no heat index and the temperatu |
...
| f | `sun_position` | 211 | The sun's elevation in degrees, and Earth's distance in AU. pyephem when it is installed, and NOAA's algorithm |
| f | `max_solar_rad` | 293 | Clear-sky radiation, W/m2, Ryan-Stolzenbach (MIT, 1972). What a solar sensor would read with no cloud above it |
| f | `windrun` | 327 | Distance the wind covered, in the same length unit as the speed. Speed times time. The unit follows the speed' |
| f | `delta` | 338 | How much a running total went up by. Three cases that are not errors and must not be treated as one: * No prev |
| **C** | `Station` | 369 | What the formulas need to know about where the station is. |
| · | &nbsp;&nbsp;`Station.altitude_ft` | 377 |  |
| **C** | `Deriver` | 384 | Fills in the readings that follow from other readings. Holds one piece of state: the last value of each runnin |
| · | &nbsp;&nbsp;`Deriver.wanted` | 398 |  |
| · | &nbsp;&nbsp;`Deriver.apply` | 408 |  |
| · | &nbsp;&nbsp;`Deriver.forget` | 516 |  |
...
| · | &nbsp;&nbsp;`BaseExport.send` | 107 |  |
| · | &nbsp;&nbsp;`BaseExport.check` | 110 |  |
| · | &nbsp;&nbsp;`BaseExport.status` | 119 |  |
| · | &nbsp;&nbsp;`BaseExport.close` | 122 |  |
| **C** | `Registry` | 126 | The exports this installation has. The same shape as the driver registry, and for the same reason: an export h |
| · | &nbsp;&nbsp;`Registry.__init__` | 134 |  |
| · | &nbsp;&nbsp;`Registry.register` | 139 |  |
| · | &nbsp;&nbsp;`Registry.register_factory` | 144 |  |
| · | &nbsp;&nbsp;`Registry.configure` | 147 |  |
| · | &nbsp;&nbsp;`Registry.factory_for` | 159 |  |
| · | &nbsp;&nbsp;`Registry.get` | 169 |  |
| · | &nbsp;&nbsp;`Registry.known` | 173 |  |
| · | &nbsp;&nbsp;`Registry.names` | 177 |  |
| · | &nbsp;&nbsp;`Registry.kinds` | 181 |  |
...
| f | `names` | 223 |  |
| f | `kinds` | 227 |  |
| f | `source_for` | 245 | Where an export's files are, from a feed name or a plain directory. A feed is asked where it wrote; a director |
| f | `walk` | 266 | Every file to consider, as paths relative to `source`. Directories that should never be uploaded are skipped h |
...
**keine Seite** · zuletzt geändert 2026-08-26 15:57
...
| · | &nbsp;&nbsp;`LocalExport.send` | 77 |  |
| · | &nbsp;&nbsp;`LocalExport.check` | 182 |  |
| · | &nbsp;&nbsp;`LocalExport.status` | 206 |  |
| · | &nbsp;&nbsp;`LocalExport.options` | 218 |  |
...
| · | &nbsp;&nbsp;`Runner.__init__` | 118 |  |
| · | &nbsp;&nbsp;`Runner.start` | 126 |  |
| · | &nbsp;&nbsp;`Runner.record_written` | 143 |  |
| · | &nbsp;&nbsp;`Runner.feed_produced` | 154 |  |
| · | &nbsp;&nbsp;`Runner.stop` | 168 |  |
| · | &nbsp;&nbsp;`Runner.status` | 213 |  |
| f | `build` | 227 | Turn configuration into things the runner can run. Anything that cannot be built is reported and left out. A m |
...
| | Name | Zeile | |
|---|---|---|---|
| **C** | `Produced` | 72 | What one run of a feed made. The list of files matters to the exports: an export that knows which files change |
| · | &nbsp;&nbsp;`Produced.count` | 87 |  |
| f | `names` | 91 | The feeds that exist. A function rather than a list, so that an export's dropdown fills itself the moment a fe |
...
[Feeds](Feeds) · zuletzt geändert 2026-08-26 15:27

| | Name | Zeile | |
|---|---|---|---|
| **C** | `JSONGenerator` | 91 | Turns plot definitions into JSON files. |
| · | &nbsp;&nbsp;`JSONGenerator.__init__` | 98 |  |
| · | &nbsp;&nbsp;`JSONGenerator.produce` | 137 |  |
| · | &nbsp;&nbsp;`JSONGenerator.build` | 250 |  |
| · | &nbsp;&nbsp;`JSONGenerator.options` | 414 |  |
| f | `from_settings` | 631 | Build the generator from the configuration. |
...
| · | &nbsp;&nbsp;`BaseDriver.status` | 83 |  |
| · | &nbsp;&nbsp;`BaseDriver.close` | 87 |  |
| f | `response_of` | 91 | What to answer an upload to this driver with. |
| f | `status_of` | 96 |  |
| **C** | `_FunctionDriver` | 107 | Wraps a plain `(body, meta) -> packets` function. |
| · | &nbsp;&nbsp;`_FunctionDriver.__init__` | 110 |  |
| · | &nbsp;&nbsp;`_FunctionDriver.packets` | 115 |  |
| **C** | `Registry` | 154 | The drivers this installation has. Instances, not classes: a driver holds configuration and state -- which con |
| · | &nbsp;&nbsp;`Registry.__init__` | 162 |  |
| · | &nbsp;&nbsp;`Registry.register` | 168 |  |
| · | &nbsp;&nbsp;`Registry.register_factory` | 175 |  |
| · | &nbsp;&nbsp;`Registry.configure` | 189 |  |
| · | &nbsp;&nbsp;`Registry.get` | 205 |  |
| · | &nbsp;&nbsp;`Registry.known` | 209 |  |
| · | &nbsp;&nbsp;`Registry.names` | 213 |  |
| · | &nbsp;&nbsp;`Registry.canonical_names` | 217 |  |
...
| | Name | Zeile | |
|---|---|---|---|
| **C** | `Ingest` | 45 | What the listener does with an upload once it has one. Kept separate from the transports so the same object se |
| · | &nbsp;&nbsp;`Ingest.__init__` | 52 |  |
| · | &nbsp;&nbsp;`Ingest.authorised` | 76 |  |
| · | &nbsp;&nbsp;`Ingest.driver_for` | 88 |  |
| · | &nbsp;&nbsp;`Ingest.submit` | 95 |  |
| · | &nbsp;&nbsp;`Ingest.status` | 182 |  |
| **C** | `_Handler` | 209 |  |
| · | &nbsp;&nbsp;`_Handler.log_message` | 213 |  |
| · | &nbsp;&nbsp;`_Handler.do_POST` | 266 |  |
| · | &nbsp;&nbsp;`_Handler.do_GET` | 283 |  |
...
| f | `push` | 408 | Send packets to a listener. This is how a pull driver delivers. Going over the loopback rather than writing to |
...
| f | `parse_json` | 57 | weewx-evo's own envelope. This is the contract every driver can rely on. One object or a list of them: {"dateT |
| f | `get` | 121 |  |
| f | `names` | 126 |  |
| f | `known` | 131 |  |
...
| f | `load` | 37 | Register every bundled driver. Returns the names that loaded. A driver that will not import is reported and sk |
...
| | Name | Zeile | |
|---|---|---|---|
| f | `schema_fields` | 27 | The fields the standard schema already has. weewx-evo carries the same wview_extended table WeeWX seeds a data |
| f | `missing` | 37 | Return the columns a packet needs and the database does not have. Args: packet (dict): A loop packet, i.e. wha |
| f | `commands` | 59 | Render the columns as the commands that create them. Unchanged from the WeeWX build of this driver: a database |
| f | `evo_commands` | 69 | The same, for weewx-evo, which adds columns itself. |
| f | `occupied` | 76 | Return {field: (count, last timestamp)} for archive columns that hold data. This is what stands between a driv |
...
| | Name | Zeile | |
|---|---|---|---|
| f | `path_for` | 58 | Where to keep the fallback file. Beside the database, because that is a directory the service writes to as its |
| **C** | `Store` | 73 | Reads and writes the list, from the database if there is one. The database is asked first and written first. T |
| · | &nbsp;&nbsp;`Store.__init__` | 80 |  |
| · | &nbsp;&nbsp;`Store.read` | 96 |  |
| · | &nbsp;&nbsp;`Store.add` | 104 |  |
| **C** | `_StateMetadata` | 162 | A weewx-evo driver state, shaped like the WeeWX manager Store expects. The state is get/set/delete on strings  |
| · | &nbsp;&nbsp;`_StateMetadata.__init__` | 171 |  |
...
| · | &nbsp;&nbsp;`EcowittDriver.options` | 114 |  |
| · | &nbsp;&nbsp;`EcowittDriver.packets` | 174 |  |
| · | &nbsp;&nbsp;`EcowittDriver.redact` | 199 |  |
| · | &nbsp;&nbsp;`EcowittDriver.status` | 208 |  |
| · | &nbsp;&nbsp;`EcowittDriver.unit_groups` | 221 |  |
| · | &nbsp;&nbsp;`EcowittDriver.missing_columns` | 233 |  |
...
| | Name | Zeile | |
|---|---|---|---|
| **C** | `Mapper` | 35 | Turns raw readings into a WeeWX packet. Args: extensions (dict): Raw field -> WeeWX field, overriding the cata |
| · | &nbsp;&nbsp;`Mapper.__init__` | 51 |  |
| · | &nbsp;&nbsp;`Mapper.to_packet` | 82 |  |
| · | &nbsp;&nbsp;`Mapper.wanted_groups` | 187 |  |
...
| | Name | Zeile | |
|---|---|---|---|
| f | `parse` | 35 | Split a payload into raw name/value pairs. Works for both protocols, because a urlencoded body and a query str |
| f | `device_time` | 61 | Return the timestamp the device sent, or None if it is not usable. Consoles are frequently wrong about the tim |
| f | `numbers` | 109 | Split raw values into the numeric ones and the rest. Returns (readings, text), where readings holds everything |
| f | `redact` | 138 | Replace the values that identify a station, leaving the readings alone. A payload is going to be pasted into a |
| f | `station_id` | 150 | What identifies the console that sent this. The PASSKEY for the Ecowitt protocol, the station ID for Weather U |
...
| · | &nbsp;&nbsp;`NoState.set` | 79 |  |
| · | &nbsp;&nbsp;`NoState.delete` | 82 |  |
| **C** | `ArchiveState` | 86 | Backed by the archive's metadata table. The right place for it: the table sits with the readings the state pro |
| · | &nbsp;&nbsp;`ArchiveState.__init__` | 97 |  |
| · | &nbsp;&nbsp;`ArchiveState.get` | 101 |  |
| · | &nbsp;&nbsp;`ArchiveState.set` | 108 |  |
| · | &nbsp;&nbsp;`ArchiveState.delete` | 114 |  |
| **C** | `FileState` | 118 | Backed by a JSON file, for when there is no database to ask. Used by the listener running on its own, and by t |
...
| f | `inspect_source` | 274 | What a driver's code reaches for, as {note: [files]}. Read, not run. This is a hint and not a guarantee: it wi |
...
| **C** | `Policy` | 61 | The aggregation policy for a whole installation. `overrides` mirrors the `[Accumulator]` section of weewx.conf |
| · | &nbsp;&nbsp;`Policy.__init__` | 70 |  |

## `src/weewx_evo/options.py`

[Konfiguration](Configuration) · zuletzt geändert 2026-08-26 15:55
...
| f | `split_duration` | 212 | Seconds as (amount, unit), in the largest unit that stays whole. 604800 becomes (7, "d"), not (604800, "s"). A |
...
| f | `defined_feeds` | 341 | The feeds that exist, for an export to point at. An export can only send what something produced, so the list  |
| f | `core_options` | 354 | What weewx-evo itself is configured with. |
...
| · | &nbsp;&nbsp;`Imported.report` | 400 |  |
| f | `labels_from` | 425 | What a WeeWX skin calls each reading: `[Labels] [[Generic]]`. Worth carrying across on its own. Somebody who r |
| f | `from_image_generator` | 441 | Plots out of a WeeWX `[ImageGenerator]` section. The structure there is three deep: a group of plots (`[[day_i |
...
| | Name | Zeile | |
|---|---|---|---|
| **C** | `Bucket` | 44 | One address's allowance. A token bucket, which is the simple one. Tokens accrue at `rate` a second up to `burs |
| · | &nbsp;&nbsp;`Bucket.__init__` | 55 |  |
| · | &nbsp;&nbsp;`Bucket.take` | 59 |  |
...
| · | &nbsp;&nbsp;`Series.empty` | 114 |  |
| · | &nbsp;&nbsp;`Series.rounded` | 118 |  |
| **C** | `Reader` | 135 | Reads series from one archive database. Holds a connection and nothing else -- no cache, and so nothing that c |
| · | &nbsp;&nbsp;`Reader.__init__` | 142 |  |
| · | &nbsp;&nbsp;`Reader.columns` | 152 |  |
| · | &nbsp;&nbsp;`Reader.has_daily` | 160 |  |
| · | &nbsp;&nbsp;`Reader.span` | 172 |  |
| · | &nbsp;&nbsp;`Reader.series` | 183 |  |
| · | &nbsp;&nbsp;`Reader.aggregate` | 214 |  |
| · | &nbsp;&nbsp;`Reader.vector` | 299 |  |
| · | &nbsp;&nbsp;`Reader.buckets` | 602 |  |
| **C** | `_NotThere` | 656 | Tells 'the summaries cannot answer' from 'the answer is nothing'. |
...
| · | &nbsp;&nbsp;`Settings.view` | 190 |  |
| · | &nbsp;&nbsp;`Settings.reload` | 206 |  |
| · | &nbsp;&nbsp;`Settings.explain` | 231 |  |
| f | `load` | 241 | Build a Settings from the places there are. A missing configuration file is not an error: a first start has no |
...
| · | &nbsp;&nbsp;`Rule.matches` | 47 |  |
| **C** | `Policy` | 52 | Which source wins for which field. Rules are checked in order and the first matching pattern decides, so put t |
| · | &nbsp;&nbsp;`Policy.from_config` | 75 |  |
| · | &nbsp;&nbsp;`Policy.is_empty` | 84 |  |
| · | &nbsp;&nbsp;`Policy.order_for` | 87 |  |
| · | &nbsp;&nbsp;`Policy.choose` | 94 |  |
| f | `sources_by_field` | 117 | Which sources carried which field, across a list of packets. |
| f | `apply` | 126 | Thin the packets down to the winning source for each field. Returns the packets with losing fields removed, an |
...
| | Name | Zeile | |
|---|---|---|---|
| f | `solar` | 54 | The sun's declination, the equation of time, and Earth's distance. Declination in degrees, the equation of tim |
| f | `position` | 108 | The sun's elevation in degrees, and Earth's distance in AU. |
| f | `solar_noon` | 140 | The moment the sun is highest, on the sun's day containing `when`. Two corrections to clock noon: how far east |
| f | `crossings` | 161 | When the sun passes an elevation on the way up and on the way down. Both times are around the solar noon neare |
| f | `events` | 218 | Sunrise, sunset and the civil twilight around them, for one day. Uses pyephem where it is installed: it accoun |
| f | `day_night` | 290 | What a chart needs to shade the hours of darkness. Returns which of day or night the span begins in, every cro |
...
[Einheiten](Units) · zuletzt geändert 2026-08-26 14:47
...
| f | `group_of` | 746 | Which group a reading belongs to, or None if nothing knows. `extra` is what a driver contributed. It wins over |
| f | `unit_of` | 764 | The unit a reading is stored in, and its group. This is the unit as it sits in the database -- what the record |
| f | `convert` | 777 | One value from one unit to another. None stays None: a gap in the readings is not zero degrees. An unknown con |
...
| f | `formatted` | 828 | A value as a person would read it. Not used by the JSON feed -- a machine reading it wants the number -- but a |
| **C** | `Target` | 844 | What to show readings in, whatever the database holds them in. A console in Germany reporting Fahrenheit and a |
| · | &nbsp;&nbsp;`Target.__init__` | 860 |  |
| · | &nbsp;&nbsp;`Target.unit` | 872 |  |
| · | &nbsp;&nbsp;`Target.for_obs` | 880 |  |
| · | &nbsp;&nbsp;`Target.convert` | 886 |  |
...
[Web-Server](Web-Server) · zuletzt geändert 2026-08-26 14:17
...
| **C** | `_Handler` | 177 |  |
| · | &nbsp;&nbsp;`_Handler.log_message` | 183 |  |
| · | &nbsp;&nbsp;`_Handler.do_GET` | 186 |  |
| · | &nbsp;&nbsp;`_Handler.do_HEAD` | 189 |  |
| **C** | `WebServer` | 288 | The local web server for the feeds. |
| · | &nbsp;&nbsp;`WebServer.__init__` | 291 |  |
| · | &nbsp;&nbsp;`WebServer.serve_forever` | 306 |  |
| · | &nbsp;&nbsp;`WebServer.start` | 309 |  |
| · | &nbsp;&nbsp;`WebServer.stop` | 314 |  |
| f | `site_from` | 319 | Build the site from the configuration. Feeds do not exist yet, so what is served comes from `[web.serve]`: nam |
...
| | Name | Zeile | |
|---|---|---|---|
| f | `parse` | 38 | A weewx.conf as nested dictionaries. Values stay strings, or become lists of strings where the file has commas |
| f | `read` | 135 |  |
